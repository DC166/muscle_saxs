...
这段代码是一个 Python 脚本，用于从图像中定位和识别峰值。它的主要步骤如下：

对图像进行平滑处理，并在非掩蔽区域中找到局部极大值。
根据阈值、接近边缘和接近阻挡器等条件掩蔽图像区域，使峰值仅在感兴趣的区域中出现。
通过距离优化构建穿过阻挡区域中心的角度线，并对距离进行分类，根据线的角度将峰值分类为两组。
通过改变线的角度来最小化残差，并对最优化线的距离进行分类，根据线的角度将峰值分类为两组。
将峰值按与中心的距离排序为一对对，用于下一步分析。
此外，脚本还包括一些帮助函数，例如从图像中提取特定区域的 ROI，计算峰值高度等。这些函数的作用是处理和分析找到的峰值。
